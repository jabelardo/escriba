...
import { X as LuX, ChevronDown as LuChevronDown, Check as LuCheck } from 'lucide-react'
...
export const PromptCombobox = ({
  ...
  const updateFilteredItems = useCallback((value: string) => {
    if (!value.trim()) {
  ...
  const handleSelectItem = useCallback((prompt: Prompt) => {
  ...
  const showNewPromptOption = inputValue.trim() && !items.find(item => item.id === inputValue)
          ...
          >
            {showNewPromptOption && (
              <Box
                ...
                onClick={() => handleSelectItem({ id: inputValue.trim(), value: '' })}
              >
            ...
            {filteredItems.map((item) => (
              <Box
                ...
                style={{
                  cursor: 'pointer',
                  borderRadius: 'var(--radius-2)',
                  backgroundColor: selectedPrompt?.id === item.id ? 'var(--accent-3)' : 'transparent'
                }}
