...
const AddOrCreateProjectDialog = () => {
    ...
    const handleAdd = async () => {
      ...
      if (missingFolders.length > 0) {
        ...
        if (exists) {
          warningToast(`${owner}/${repo} is already registered.`, 'Project already added')
        } else {
          addProject({ owner, repo })
          errorToast(`${owner}/${repo} has been added successfully.`, 'Project added')
        }
        setInput('')
      }
      setLoading(false)
    ...
    const AddExistingProject = () => (
        <Flex gap="3" pt="3">
         ...
         <Dialog.Root open={openMissingFoldersDialog} onOpenChange={(e) => setOpenMissingFoldersDialog(e)}>
           <Dialog.Content>
    ...
    return (
      <>
        <Dialog.Root>
            <Dialog.Trigger>
            <Button size='2' variant="ghost">+ Add / Create Project</Button>
            </Dialog.Trigger>
            <Dialog.Content>
